...
- Pull request: #1
- Current PR head pinned for this review: `ada6cf01cc922bcfff403b90bb628eec05ee93e2`
- Current PR base pinned for this review: `d8a05d37e3cbd58940af65a22b4c654e56e35781`
- Previous correction artifact blob SHA: `9a4207e420c2e4e82ee722cde2fa266c60c4ff12`
...
**Normative resolution**: At `docs/ISSUE_PLAN.md:112`, the canonical contract SHALL make this requirement normative: This row omits Issue 09 and Issue 14 even though docs/issues/13-scenario-store.md requires size/depth limiting and migration before loadScenario is complete. If the authoritative dependency table is used to generate or schedule the work, the scenario store can be implemented before its required security limits and migration path exist, leading to skipped checks or placeholder unknown-version handling. Add the missing prerequisites here to keep the plan executable.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/24-mock-server-adapter.md:18`, the canonical contract SHALL make this requirement normative: The mock adapter only registers initialize , tools/list , and tools/call , so a downstream MCP client that sends the core ping request after connecting will fall into the unmatched path instead of receiving the required empty result. The research note already marks ping as MVP/pass-through, so add an explicit ping handler and test here to avoid healthy clients treating the mock as stale.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/17-mcp-client-wrapper.md:34`, the canonical contract SHALL make this requirement normative: The detailed requirements pull per-request timeout handling from Issue 09, but the dependency list only names 06/07/11. That leaves the wrapper spec under-constrained.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/19-recorder-core.md:35`, the canonical contract SHALL make this requirement normative: Line 24 depends on the timeout contract from Issue 09, but the dependency set stops at 17/18/20/21. Add 09 so the recorder state machine cannot be scheduled before its timeout semantics exist.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/21-redaction-engine.md:35`, the canonical contract SHALL make this requirement normative: env-values is explicitly structural and depends on the env allowlist work from Issue 07, but the Dependencies block only names 11. As written, this can be scheduled before its prerequisite exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/29-run-command.md:31`, the canonical contract SHALL make this requirement normative: The command scope explicitly says it must load and validate scripts via Issue 26, but the dependency list skips it. That hides a hard prerequisite and makes the command spec look implementable before the parser/validator exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/40-prompts-record.md:25`, the canonical contract SHALL make this requirement normative: This spec depends on both Issue 18 for notifications/prompts/list changed capture and Issue 21 for redaction of prompt payloads, but neither is listed. Without them, this work can be scheduled before the notification/redaction plumbing exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/41-prompts-replay.md:26`, the canonical contract SHALL make this requirement normative: This replay spec consumes the prompts catalog shape and recorded prompt interactions introduced by the recorder work in Issue 40. Keeping that dependency implicit makes the replay task look implementable before the data model exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/43-roots.md:30`, the canonical contract SHALL make this requirement normative: The scope includes roots/list changed notifications, but the dependency list only names Issues 17 and 24. Issue 18 is the piece that captures bidirectional notifications, so this work can be picked up before that plumbing exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
...
**Normative resolution**: At `docs/issues/44-elicitation.md:30`, the canonical contract SHALL make this requirement normative: This spec explicitly says elicited values are subject to redaction, but the dependency list stops at 17/18/24. Add the redaction engine issue so this work is not scheduled before the no-secret-leak path exists.. Unsafe, malformed, or unsupported inputs SHALL fail at the stated boundary without leaking data or leaving an unintended partial side effect.
